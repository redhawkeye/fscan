package Plugins

import (
	"encoding/binary"
	"encoding/hex"
	"fmt"
	"github.com/shadow1ng/fscan/Common"
	"os"
	"strings"
	"time"
)

var (
	// Encrypted SMB protocol request data
	negotiateProtocolRequest_enc  = ""
	sessionSetupRequest_enc       = ""
	treeConnectRequest_enc        = ""
	transNamedPipeRequest_enc     = "k/RGiUQ/tw1yiqioUIqirzGC1SxTAmQmtnfKd1qiLish7FQYxvE+h4/p7RKgWemIWRXDf2XSJ3K0LUIX0vv1gx2eb4NatU7Qosnrhebz3gUo7u25P5BZH1QKdagzPqtitVjASpxIjB3uNWtYMrXGkkuAm8QEitberc+mP0vnzZ8Nv/xiiGBko8O4P/wCKaN2KZVDLbv2jrN8V/1zY6fvWA=="
	trans2SessionSetupRequest_enc = "JqNw6PUKcWOYFisUoUCyD24wnML2Yd8kumx9hJnFWbhM2TQkRvKHsOMWzPVfggRrLl8sLQFqzk8bv8Rpox3uS61l480Mv7HdBPeBeBeFudZMntXBUa4pWUH8D9EXCjoUqgAdvw6kGbPOOKUq3WmNb0GDCZapqQwyUKKMHmNIUMVMAOyVfKeEMJA6LViGwyvHVMNZ1XWLr0xafKfEuz4qoHiDyVWomGjJt8DQd6+jgLk="

	// Decrypted SMB protocol request data
	negotiateProtocolRequest  []byte
	sessionSetupRequest       []byte
	treeConnectRequest        []byte
	transNamedPipeRequest     []byte
	trans2SessionSetupRequest []byte
)

func init() {
	var err error

	// Decrypt protocol request
	decrypted, err := AesDecrypt(negotiateProtocolRequest_enc, key)
	if err != nil {
		Common.LogError(fmt.Sprintf("Protocol request decryption error: %v", err))
		os.Exit(1)
	}
	negotiateProtocolRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("Protocol request decoding error: %v", err))
		os.Exit(1)
	}

	// Decrypt session request
	decrypted, err = AesDecrypt(sessionSetupRequest_enc, key)
	if err != nil {
		Common.LogError(fmt.Sprintf("Session request decryption error: %v", err))
		os.Exit(1)
	}
	sessionSetupRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("Session request decoding error: %v", err))
		os.Exit(1)
	}

	// Decrypt connection request
	decrypted, err = AesDecrypt(treeConnectRequest_enc, key)
	if err != nil {
		Common.LogError(fmt.Sprintf("Connection request decryption error: %v", err))
		os.Exit(1)
	}
	treeConnectRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("Connection request decoding error: %v", err))
		os.Exit(1)
	}

	// Decrypt pipe request
	decrypted, err = AesDecrypt(transNamedPipeRequest_enc, key)
	if err != nil {
		Common.LogError(fmt.Sprintf("Pipe request decryption error: %v", err))
		os.Exit(1)
	}
	transNamedPipeRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("Pipe request decoding error: %v", err))
		os.Exit(1)
	}

	// Decrypt session setup request
	decrypted, err = AesDecrypt(trans2SessionSetupRequest_enc, key)
	if err != nil {
		Common.LogError(fmt.Sprintf("Session setup decryption error: %v", err))
		os.Exit(1)
	}
	trans2SessionSetupRequest, err = hex.DecodeString(decrypted)
	if err != nil {
		Common.LogError(fmt.Sprintf("Session setup decoding error: %v", err))
		os.Exit(1)
	}
}

// MS17010 scan entry function
func MS17010(info *Common.HostInfo) error {
	if Common.DisableBrute {
		return nil
	}

	err := MS17010Scan(info)
	if err != nil {
		Common.LogError(fmt.Sprintf("%s:%s - %v", info.Host, info.Ports, err))
	}
	return err
}

func MS17010Scan(info *Common.HostInfo) error {
	ip := info.Host

	// Connect to target
	conn, err := Common.WrapperTcpWithTimeout("tcp", ip+":445", time.Duration(Common.Timeout)*time.Second)
	if err != nil {
		return fmt.Errorf("Connection error: %v", err)
	}
	defer conn.Close()

	if err = conn.SetDeadline(time.Now().Add(time.Duration(Common.Timeout) * time.Second)); err != nil {
		return fmt.Errorf("Set timeout error: %v", err)
	}

	// SMB protocol negotiation
	if _, err = conn.Write(negotiateProtocolRequest); err != nil {
		return fmt.Errorf("Send protocol request error: %v", err)
	}

	reply := make([]byte, 1024)
	if n, err := conn.Read(reply); err != nil || n < 36 {
		if err != nil {
			return fmt.Errorf("Read protocol response error: %v", err)
		}
		return fmt.Errorf("Incomplete protocol response")
	}

	if binary.LittleEndian.Uint32(reply[9:13]) != 0 {
		return fmt.Errorf("Protocol negotiation rejected")
	}

	// Establish session
	if _, err = conn.Write(sessionSetupRequest); err != nil {
		return fmt.Errorf("Send session request error: %v", err)
	}

	n, err := conn.Read(reply)
	if err != nil || n < 36 {
		if err != nil {
			return fmt.Errorf("Read session response error: %v", err)
		}
		return fmt.Errorf("Incomplete session response")
	}

	if binary.LittleEndian.Uint32(reply[9:13]) != 0 {
		return fmt.Errorf("Session establishment failed")
	}

	// Extract system information
	var os string
	sessionSetupResponse := reply[36:n]
	if wordCount := sessionSetupResponse[0]; wordCount != 0 {
		byteCount := binary.LittleEndian.Uint16(sessionSetupResponse[7:9])
		if n != int(byteCount)+45 {
			Common.LogError(fmt.Sprintf("Invalid session response %s:445", ip))
		} else {
			for i := 10; i < len(sessionSetupResponse)-1; i++ {
				if sessionSetupResponse[i] == 0 && sessionSetupResponse[i+1] == 0 {
					os = string(sessionSetupResponse[10:i])
					os = strings.Replace(os, string([]byte{0x00}), "", -1)
					break
				}
			}
		}
	}

	// Tree connect request
	userID := reply[32:34]
	treeConnectRequest[32] = userID[0]
	treeConnectRequest[33] = userID[1]

	if _, err = conn.Write(treeConnectRequest); err != nil {
		return fmt.Errorf("Send tree connect request error: %v", err)
	}

	if n, err := conn.Read(reply); err != nil || n < 36 {
		if err != nil {
			return fmt.Errorf("Read tree connect response error: %v", err)
		}
		return fmt.Errorf("Incomplete tree connect response")
	}

	// Named pipe request
	treeID := reply[28:30]
	transNamedPipeRequest[28] = treeID[0]
	transNamedPipeRequest[29] = treeID[1]
	transNamedPipeRequest[32] = userID[0]
	transNamedPipeRequest[33] = userID[1]

	if _, err = conn.Write(transNamedPipeRequest); err != nil {
		return fmt.Errorf("Send pipe request error: %v", err)
	}

	if n, err := conn.Read(reply); err != nil || n < 36 {
		if err != nil {
			return fmt.Errorf("Read pipe response error: %v", err)
		}
		return fmt.Errorf("Incomplete pipe response")
	}

	// Vulnerability detection part adds Output
	if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
		// Construct basic details
		details := map[string]interface{}{
			"port":          "445",
			"vulnerability": "MS17-010",
		}
		if os != "" {
			details["os"] = os
			Common.LogSuccess(fmt.Sprintf("Vulnerability found %s [%s] MS17-010", ip, os))
		} else {
			Common.LogSuccess(fmt.Sprintf("Vulnerability found %s MS17-010", ip))
		}

		// Save MS17-010 vulnerability result
		result := &Common.ScanResult{
			Time:    time.Now(),
			Type:    Common.VULN,
			Target:  ip,
			Status:  "vulnerable",
			Details: details,
		}
		Common.SaveResult(result)

		// DOUBLEPULSAR backdoor detection
		trans2SessionSetupRequest[28] = treeID[0]
		trans2SessionSetupRequest[29] = treeID[1]
		trans2SessionSetupRequest[32] = userID[0]
		trans2SessionSetupRequest[33] = userID[1]

		if _, err = conn.Write(trans2SessionSetupRequest); err != nil {
			return fmt.Errorf("Send backdoor detection request error: %v", err)
		}

		if n, err := conn.Read(reply); err != nil || n < 36 {
			if err != nil {
				return fmt.Errorf("Read backdoor detection response error: %v", err)
			}
			return fmt.Errorf("Incomplete backdoor detection response")
		}

		if reply[34] == 0x51 {
			Common.LogSuccess(fmt.Sprintf("Backdoor found %s DOUBLEPULSAR", ip))

			// Save DOUBLEPULSAR backdoor result
			backdoorResult := &Common.ScanResult{
				Time:   time.Now(),
				Type:   Common.VULN,
				Target: ip,
				Status: "backdoor",
				Details: map[string]interface{}{
					"port": "445",
					"type": "DOUBLEPULSAR",
					"os":   os,
				},
			}
			Common.SaveResult(backdoorResult)
		}

		// Shellcode exploitation part remains unchanged
		if Common.Shellcode != "" {
			defer MS17010EXP(info)
		}
	} else if os != "" {
		Common.LogInfo(fmt.Sprintf("System information %s [%s]", ip, os))

		// Save system information
		sysResult := &Common.ScanResult{
			Time:   time.Now(),
			Type:   Common.SERVICE,
			Target: ip,
			Status: "identified",
			Details: map[string]interface{}{
				"port":    "445",
				"service": "smb",
				"os":      os,
			},
		}
		Common.SaveResult(sysResult)
	}

	return nil
}
